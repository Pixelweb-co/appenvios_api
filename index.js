...
usuariom.findOneAndUpdate({_id:req.body.user}, {password:req.body.password}, {upsert:true}, function(err, results){
...
usuariom.findOneAndUpdate({_id:req.body.user}, {nombres:req.body.nombres,telefono:req.body.telefono,cedula:req.body.cedula}, {upsert:true}, function(err, results){
...
require("fs").writeFile("public/uploads/"+req.body.user+".png", base64Data, 'base64', function(err) {
  console.log(err);

if(!err){

       usuariom.findOneAndUpdate({_id:req.body.user}, {avatar:req.body.user+".png"}, {upsert:true}, function(err, results){

     return res
            .status(200)
            .send({success:true,image:'uploads/'+req.body.user+".png"});
...
app.post('/api/set_leidos',function(req,res){
       ...
       Mensajes.findOneAndUpdate({solicitud:req.body.solicitud, para:req.body.user}, {leido:true}, {upsert:true}, function(err, results){
...
usuariom.find({placa_vehiculo:req.body.placa_vehiculo},function(err, user) {
            if(err){
                res.send(err);
            }
...
var pickedf = _.filter(usernames, { 'userName': socket.handshake.query.cliente } );
...
if(solicitudes_rooms.length == 0){
        ...
        Solicitud.find({estado:'En curso'},function(err, solicitudes_en_curso) {
			...
			var picked_contratante_enlinea = _.filter(usernames, { 'userName': item.contratante } );
			var picked_contratista_enlinea = _.filter(usernames, { 'userName': item.contratista } );

			//esta en linea?
			if(picked_contratante_enlinea > 0){

				var clients_socket = io.sockets.clients(picked_contratista_enlinea.id); // list of all clients in room1
...
var picked_solicitud_existe_cte = _.filter(solicitudes_rooms, { 'contratante': socket.handshake.query.cliente } );

//contratista
var picked_solicitud_existe_cta = _.filter(solicitudes_rooms, { 'contratista': socket.handshake.query.cliente } );
...
if(picked_solicitud_existe_cte.length == 0){
	...
	if(picked_solicitud_existe_cta == 0){
		...
		console.log('contratista en solicitud guardada reconectando ... ',picked_solicitud_existe_cta.sala);
		...
		console.log('contratante en solicitud guardada reconectando ... ',picked_solicitud_existe_cte.sala);
...
var picked_sala = _.filter(solicitudes_rooms, { 'sala': 'solicitud_'+dataf.id } );
...
socket.emit('solicitud_cliente',{success: 'SE encuentra',solicitud: sol});
...
if(data.id){
Solicitud.findOneAndUpdate({"_id": data.id}, {estado:"Abierta",contratista:undefined}, {upsert:true}, function(err, results){
  ...
  Solicitudes_canceladas.create({
    ...
    Solicitud.findOne({_id:data.id}, function (err, sol) {
        ...
        usuariom.findOne({_id:data.contratista}, function (err, contratista_c) {
...
socket.broadcast.to('contratantes').emit('cancelada', { solicitud: sol, contratista: contratista_c, texto_cancelacion:data.detalle.texto_cancelacion});
...
Solicitud.findOne({_id: data.solicitud._id, estado:"Abierta"}, function(err, sol){
...
usuariom.findOne({_id: socket.handshake.query.cliente }, function(err, user) {
    ...
    Solicitud.findOneAndUpdate({"_id": sol._id, "estado": "Abierta"}, data.solicitud, {upsert:true}, function(err, results){
...
var picked_sala = _.filter(solicitudes_rooms, { 'sala': 'solicitud_'+sol._id} );
...
socket.broadcast.to('contratistas').emit('tomada', {user:socket.handshake.query.cliente, sol:data.solicitud._id, estado:data.solicitud.estado});
socket.broadcast.to('contratantes').emit('modal_servicio_tomado',{solicitud:data.solicitud});
socket.broadcast.to('contratantes').emit('updateapp');
...
socket.broadcast.to('contratistas').emit('solicitudes_abiertas', { data: docs });
        ...
        socket.emit('solicitud_asignacion',{success: 'Asignada correctamente.'});
...
Mensajes.find({solicitud:data._id}, function (err, docs) {  
		...
		console.log("enviando lista mensajes de usuario sala solicitud_"+data._id); 
...
Mensajes.updateMany({solicitud:data.solicitud._id, de:data.de},{leido:true}, function(err, results){
...
console.log(" set leidos enviando lista mensajes de usuario sala solicitud_ ") 
...
Mensajes.find({solicitud:data.solicitud}, function (err, docs) {  
		...
		console.log("enviando lista mensajesde sala solicitud_"+data.solicitud) 

socket.broadcast.to('solicitud_'+data.solicitud).emit('updatechat',{mensajes:docs});
...
/////////////////
        ...
        Solicitud.find({estado:'Abierta'},function(err, solicitudes) {
            ...
            socket.broadcast.to('contratistas').emit('solicitudes_abiertas', { data: solicitudes });
